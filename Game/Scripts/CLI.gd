...
func id():
	...
	for id in references:
		...
		print_text("\t\tStatus: "+(("[color=red]"+Enums.STATUS.keys()[references[id]["status"]]+"[/color]") if references[id]["status"] == Enums.STATUS.DEAD else Enums.STATUS.keys()[references[id]["status"]]))
